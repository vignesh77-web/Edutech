...
function CourseDetailsCard({ course, setConfirmationModal, handleBuyCourse }) {
  ...
  const userCourse = user?.courses?.find(c => (c.courseId?._id || c.courseId || c) === course?._id);
  const isEnrolled = !!userCourse || course?.studentsEnrolled?.some(id => (id._id || id) === user?._id);
  const isExpired = userCourse && userCourse.expiresAt && new Date(userCourse.expiresAt) < new Date();
      ...
      >
        ...
        <div className="px-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              {user && isEnrolled
                ? isExpired ? "Renew Membership" : "Go To Course"
                : "Buy Now"}
            </button>
            {(!user || (!isEnrolled || isExpired)) && (
